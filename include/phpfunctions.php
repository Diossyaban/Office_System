...
function getCustomMapIcon($field, $table, $data)
{
	...
	if(!$table)
		$table = $strTableName;
	if($table=="t_delivery_feedback" && $field=="Lat")
	{
		$icon = "images/menuicons/Boss.png";;
		return $icon;
	}
...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="t_delivery" && $field=="t_delivery_status")
	{
...
;
		return $value;
				...
				if($table=="t_delivery" && $field=="status_disposition")
	{
		$value="<span class='label label-default'>".$value."</span>"; ;
		return $value;
	}
				if($table=="m_disposition_inmail" && $field=="t_disposition_status")
	{
...
;
		return $value;
				...
				if($table=="t_delivery_x" && $field=="status")
	{
		$value="<span class='label label-hitam'>".$value."</span>"; 
;
		return $value;
	}
				if($table=="m_disposition_profile" && $field=="t_disposition_status")
	{
...
if ($value == "Followup") {  
    ...
    $value="<span class='label label-primary'>".$value."</span>"; 
}
...
;
		return $value;
				...
				if($table=="t_register" && $field=="t_register_status")
	{
		if ($value == "Registered") {
...
if ($value == "Followup") {  
    ...
    $value="<span class='label label-primary'>".$value."</span>"; 
		...
		return $value;
				...
				if($table=="t_in_mail" && $field=="t_register_status")
	{
		if ($value == "Registered") {
...
;
		return $value;
				...
				if($table=="m_disposition_register" && $field=="t_disposition_status")
	{
...
;
		return $value;
				...
				if($table=="m_disposition_inmail_x" && $field=="t_disposition_status")
	{
...
{
	$value = "";
				if($table=="m_mapping_letter" && $field=="template_file")
	{
		;
		return $value;
	}
				if($table=="m_announcement" && $field=="announcement_file")
	{
		;
		return $value;
	}
				if($table=="m_user_guide" && $field=="m_user_guide_file")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="t_letter" && $field=="add_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="t_letter" && $field=="letter_status")
	{
		return Draft;
	}
				if($table=="t_delivery" && $field=="t_select_table_text")
	{
		return "Address book";
	}
				if($table=="t_delivery" && $field=="t_delivery_status")
	{
		return 4;
				...
				if($table=="t_delivery" && $field=="add_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="t_delivery" && $field=="edit_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="t_invoice" && $field=="invoice_no")
	{
		return strftime("%Y.%m.%d.%H.%M.%S");
	}
				if($table=="t_meeting_book" && $field=="t_meeting_to_date")
	{
		return $values["t_meeting_to_date"] = date('Y-m-d');


if (isset($_POST['time_field'])) {
    $selectedTime = $_POST['time_field'];
    $values["t_meeting_to_date"] .= ' ' . $selectedTime;
}
;
	}
				if($table=="m_disposition" && $field=="t_disposition_from")
	{
		return $userData = Security::getUserName();;
	}
	return "";
}
